    ...
    <div className="min-h-full p-6" style={{ background: '#f5f7fb' }}>
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-7">
          <div>
            <h1 className="text-lg font-bold text-gray-900 tracking-tight">Nouveau rapport</h1>
            <p className="text-xs text-slate-400 mt-0.5">Importez un procès-verbal Atterberg manuscrit au format PDF</p>
          </div>
          <button
            onClick={() => navigate('/')}
            className="flex items-center gap-1.5 text-xs font-medium text-slate-500 hover:text-slate-700 px-3 py-2 rounded-xl border border-slate-200 hover:bg-slate-50 transition-all">
        ...
        <div className="mb-4" style={CARD}>
          ...
          <div
            ...
            style={{
              borderColor: dragging ? '#4f6ef7' : file ? '#22c55e' : '#e2e8f0',
              background: dragging ? 'rgba(79,110,247,0.04)' : file ? 'rgba(34,197,94,0.03)' : '#fafbff',
          ...
          >
            ...
            <div className="py-14 px-8 flex flex-col items-center text-center">
              {file ? (
                <>
                  <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-4 bg-green-50">
                  ...
                  <p className="text-sm text-slate-400 mb-4">{(file.size / 1024).toFixed(0)} Ko · PDF</p>
                  <button
                    onClick={e => { e.stopPropagation(); setFile(null) }}
                    className="text-xs font-medium px-3 py-1.5 rounded-lg text-red-400 hover:text-red-600 border border-red-100 hover:border-red-300 hover:bg-red-50 transition-all">
                    Changer de fichier
                  </button>
                </>
              ) : (
                <>
                  <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-5 transition-colors"
                    style={{ background: dragging ? 'rgba(79,110,247,0.1)' : '#f1f5f9' }}>
                    <svg className="w-8 h-8 transition-colors"
                      style={{ color: dragging ? '#4f6ef7' : '#94a3b8' }}
                  ...
                  <p className="text-sm text-slate-400 mt-1.5">
                    ou <span className="text-brand-600 font-semibold hover:text-brand-700 transition-colors">cliquez pour parcourir</span>
                  </p>
                  <div className="mt-5 flex items-center gap-2">
                    {['PDF uniquement', 'Max 10 Mo'].map(label => (
                      <span key={label} className="inline-flex items-center gap-1.5 text-xs text-slate-300 px-3 py-1 rounded-full border border-slate-100">
        ...
        <div className="grid grid-cols-3 gap-3">
          {STEPS.map(step => (
            <div key={step.n} className="p-4" style={CARD}>
              <div className="flex items-center gap-2 mb-3">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: step.bg, color: step.color }}>
                  {step.icon}
                </div>
